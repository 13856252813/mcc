...
import com.txt.library.model.ChatSend;
...
public class SystemChat extends SystemBase implements KandyChatServiceNotificationListener {
     ...
     */
    public void sendImage(Uri imgUri,KandyUploadProgressListener listener) {
        Log.i(TAG, "---uri:" + imgUri + "------agentCall:" + getSystem(SystemKandy.class).mAgentCall);
        KandyRecord recipient = null;
        recipient = getRecipient(getSystem(SystemKandy.class).mAgentCall, false);
        if (recipient == null)
            return;
        IKandyImageItem kandyImage = null;
        try {
            kandyImage = KandyMessageBuilder.createImage("", imgUri);
        } catch (KandyIllegalArgumentException e) {
            Log.d(TAG, "sendImage: " + e.getLocalizedMessage(), e);
        }
        final KandyChatMessage message = new KandyChatMessage(recipient, kandyImage);
        Kandy.getServices().getChatService().sendChat(message,listener);
//
//        Kandy.getServices().getChatService().sendChat(message, new KandyUploadProgressListener() {
//
//            @Override
